...
// 创建中文版本的 VSIX 包
async function createChineseVSIX() {
    const zip = new JSZip();
    ...
    // 读取中文本地化文件来替换占位符
    const nlsPath = path.join(__dirname, 'package.nls.zh-cn.json');
    if (fs.existsSync(nlsPath)) {
        ...
        console.log('✅ Replaced Chinese localization placeholders');
    }
    ...
    const vsixName = `node_modules-url-extractor-0.1.0-zh.vsix`;
    fs.writeFileSync(vsixName, content);
    
    console.log(`✅ Chinese VSIX package created: ${vsixName}`);
...
createChineseVSIX().catch(console.error);